...
GST_DEBUG_CATEGORY_STATIC (gst_vtdec_265_debug_category);
#define GST_CAT_DEFAULT gst_vtdec_265_debug_category
...
static OSStatus gst_vtdec_create_session (GstVtdec265 * vtdec,
    GstVideoFormat format, gboolean enable_hardware);
static void gst_vtdec_invalidate_session (GstVtdec265 * vtdec);
static CMSampleBufferRef cm_sample_buffer_from_gst_buffer (GstVtdec265 * vtdec,
    GstBuffer * buf);
static GstFlowReturn gst_vtdec_push_frames_if_needed (GstVtdec265 * vtdec,
    gboolean drain, gboolean flush);
static CMFormatDescriptionRef create_format_description (GstVtdec265 * vtdec,
    CMVideoCodecType cm_format);
static CMFormatDescriptionRef
create_format_description_from_codec_data (GstVtdec265 * vtdec,
...
static void gst_vtdec_session_output_callback (void
    *decompression_output_ref_con, void *source_frame_ref_con, OSStatus status,
    ...
    CMTime duration);
static gboolean compute_h264_decode_picture_buffer_length (GstVtdec265 * vtdec,
    GstBuffer * codec_data, int *length);
static gboolean gst_vtdec_compute_reorder_queue_length (GstVtdec265 * vtdec,
    CMVideoCodecType cm_format, GstBuffer * codec_data);
static void gst_vtdec_set_latency (GstVtdec265 * vtdec);
...
static GstStaticPadTemplate gst_vtdec_sink_template =
    ...
    GST_PAD_SINK,
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS ("video/x-h265, stream-format=hvc1, alignment=au,"
...
G_DEFINE_TYPE (GstVtdec265, gst_vtdec_265, GST_TYPE_VIDEO_DECODER);

static void
gst_vtdec_265_class_init (GstVtdec265Class * klass)
{
...
static void
gst_vtdec_265_init (GstVtdec265 * vtdec)
{
...
{
  GstVtdec265 *vtdec = GST_VTDEC_265 (object);

  GST_DEBUG_OBJECT (vtdec, "finalize");

  g_async_queue_unref (vtdec->reorder_queue);

  G_OBJECT_CLASS (gst_vtdec_265_parent_class)->finalize (object);
}

static gboolean
gst_vtdec_start (GstVideoDecoder * decoder)
{
  GstVtdec265 *vtdec = GST_VTDEC_265 (decoder);
...
static void
setup_texture_cache (GstVtdec265 * vtdec)
{
...
gst_vtdec_negotiate (GstVideoDecoder * decoder)
{
  ...
  return GST_VIDEO_DECODER_CLASS (gst_vtdec_265_parent_class)->negotiate (decoder);
}
...
gst_vtdec_set_format (GstVideoDecoder * decoder, GstVideoCodecState * state)
{
  ...
  if (!strcmp (caps_name, "video/x-h265")) {
  ...
  if (cm_format == kCMVideoCodecType_HEVC && state->codec_data == NULL) {
...
static void
gst_vtdec_invalidate_session (GstVtdec265 * vtdec)
{
...
static OSStatus
gst_vtdec_create_session (GstVtdec265 * vtdec, GstVideoFormat format,
...
static CMFormatDescriptionRef
create_format_description (GstVtdec265 * vtdec, CMVideoCodecType cm_format)
{
  OSStatus status;
...
cm_sample_buffer_from_gst_buffer (GstVtdec265 * vtdec, GstBuffer * buf)
{
  OSStatus status;
...
{
  GstVtdec265 *vtdec = (GstVtdec265 *) decompression_output_ref_con;
...
static GstFlowReturn
gst_vtdec_push_frames_if_needed (GstVtdec265 * vtdec, gboolean drain,
    gboolean flush)
{
  GstVideoCodecFrame *frame;
  GstFlowReturn ret = GST_FLOW_OK;
...
static gboolean
parse_h264_profile_and_level_from_codec_data (GstVtdec265 * vtdec,
    GstBuffer * codec_data, int *profile, int *level)
{
  ...
  gst_buffer_map (codec_data, &map, GST_MAP_READ);
    GST_MEMDUMP("CODEC DATA", map.data, map.size);
...
wrong_version:
  GST_ELEMENT_ERROR (vtdec, STREAM, DECODE, (NULL),
      ("wrong hvcC version in codec_data"));
  ret = FALSE;
  goto out;
}

static int
get_dpb_max_mb_s_from_level (GstVtdec265 * vtdec, int level)
{
  switch (level) {
    case 10:
      /* 1b?? */
...
static gboolean
gst_vtdec_compute_reorder_queue_length (GstVtdec265 * vtdec,
    CMVideoCodecType cm_format, GstBuffer * codec_data)
{
...
static gboolean
compute_h264_decode_picture_buffer_length (GstVtdec265 * vtdec,
    GstBuffer * codec_data, int *length)
{
  int profile, level;
  int dpb_mb_size = 16;
...
static void
gst_vtdec_set_latency (GstVtdec265 * vtdec)
{
...
{
  GstVtdec265 *vtdec = GST_VTDEC_265 (element);
  ...
  GST_ELEMENT_CLASS (gst_vtdec_265_parent_class)->set_context (element, context);
}

#ifndef HAVE_IOS
#define GST_TYPE_VTDEC_265_HW   (gst_vtdec_265_hw_get_type())
#define GST_VTDEC_265_HW(obj)   (G_TYPE_CHECK_INSTANCE_CAST((obj),GST_TYPE_VTDEC_265_HW,GstVtdec265Hw))
#define GST_VTDEC_265_HW_CLASS(klass)   (G_TYPE_CHECK_CLASS_CAST((klass),GST_TYPE_VTDEC_265_HW,GstVtdec265HwClass))
#define GST_IS_VTDEC_265_HW(obj)   (G_TYPE_CHECK_INSTANCE_TYPE((obj),GST_TYPE_VTDEC_265_HW))
#define GST_IS_VTDEC_265_HW_CLASS(obj)   (G_TYPE_CHECK_CLASS_TYPE((klass),GST_TYPE_VTDEC_265_HW))

typedef GstVtdec265 GstVtdec265Hw;
typedef GstVtdec265Class GstVtdec265HwClass;

GType gst_vtdec_265_hw_get_type (void);

G_DEFINE_TYPE (GstVtdec265Hw, gst_vtdec_265_hw, GST_TYPE_VTDEC_265);

static void
gst_vtdec_265_hw_class_init (GstVtdec265HwClass * klass)
{
...
static void
gst_vtdec_265_hw_init (GstVtdec265Hw * vtdec)
{
  GST_VTDEC_265 (vtdec)->require_hardware = TRUE;
}

#endif

void
gst_vtdec_265_register_elements (GstPlugin * plugin)
{
  GST_DEBUG_CATEGORY_INIT (gst_vtdec_265_debug_category, "vtdec265", 0,
      "debug category for vtdec element");

#ifdef HAVE_IOS
  gst_element_register (plugin, "vtdec265", GST_RANK_PRIMARY, GST_TYPE_VTDEC_265);
#else
  gst_element_register (plugin, "vtdec265_hw", GST_RANK_PRIMARY + 1,
      GST_TYPE_VTDEC_265_HW);
  gst_element_register (plugin, "vtdec265", GST_RANK_SECONDARY, GST_TYPE_VTDEC_265);
#endif
}
