...
GST_DEBUG_CATEGORY (gst_vtenc_265_debug);
#define GST_CAT_DEFAULT (gst_vtenc_265_debug)

#define GST_VTENC_CODEC_DETAILS_QDATA \
    g_quark_from_static_string ("vtenc256-codec-details")
...
CFSTR ("ProfileLevel");
//const CFStringRef kVTProfileLevel_HEVC_Main_AutoLevel =
//CFSTR ("H265_Main_AutoLevel");
#endif
*/
...
static void gst_vtenc_clear_cached_caps_downstream (GstVTEnc265 * self);

static VTCompressionSessionRef gst_vtenc_create_session (GstVTEnc265 * self);
static void gst_vtenc_destroy_session (GstVTEnc265 * self,
    VTCompressionSessionRef * session);
static void gst_vtenc_session_dump_properties (GstVTEnc265 * self,
    VTCompressionSessionRef session);
static void gst_vtenc_session_configure_expected_framerate (GstVTEnc265 * self,
    VTCompressionSessionRef session, gdouble framerate);
static void gst_vtenc_session_configure_max_keyframe_interval (GstVTEnc265 * self,
    VTCompressionSessionRef session, gint interval);
static void gst_vtenc_session_configure_max_keyframe_interval_duration
    (GstVTEnc265 * self, VTCompressionSessionRef session, gdouble duration);
static void gst_vtenc_session_configure_bitrate (GstVTEnc265 * self,
    VTCompressionSessionRef session, guint bitrate);
static OSStatus gst_vtenc_session_configure_property_int (GstVTEnc265 * self,
    VTCompressionSessionRef session, CFStringRef name, gint value);
static OSStatus gst_vtenc_session_configure_property_double (GstVTEnc265 * self,
    VTCompressionSessionRef session, CFStringRef name, gdouble value);
static void gst_vtenc_session_configure_allow_frame_reordering (GstVTEnc265 * self,
    VTCompressionSessionRef session, gboolean allow_frame_reordering);
static void gst_vtenc_session_configure_realtime (GstVTEnc265 * self,
    VTCompressionSessionRef session, gboolean realtime);

static GstFlowReturn gst_vtenc_encode_frame (GstVTEnc265 * self,
    GstVideoCodecFrame * frame);
...
static gboolean gst_vtenc_buffer_is_keyframe (GstVTEnc265 * self,
...
static void
gst_vtenc_base_init (GstVTEnc265Class * klass)
{
  const GstVTEncoder265Details *codec_details =
      GST_VTENC_265_CLASS_GET_CODEC_DETAILS (klass);
  ...
  if (codec_details->format_id == kCMVideoCodecType_HEVC) {
...
static void
gst_vtenc_class_init (GstVTEnc265Class * klass)
{
...
static void
gst_vtenc_init (GstVTEnc265 * self)
{
  GstVTEnc265Class *klass = (GstVTEnc265Class *) G_OBJECT_GET_CLASS (self);
  CFStringRef keyframe_props_keys[] = { kVTEncodeFrameOptionKey_ForceKeyFrame };
  CFBooleanRef keyframe_props_values[] = { kCFBooleanTrue };

  self->details = GST_VTENC_265_CLASS_GET_CODEC_DETAILS (klass);
...
{
  GstVTEnc265 *self = GST_VTENC_265_CAST (obj);
...
static guint
gst_vtenc_get_bitrate (GstVTEnc265 * self)
{
  guint result;
...
static void
gst_vtenc_set_bitrate (GstVTEnc265 * self, guint bitrate)
{
  GST_OBJECT_LOCK (self);
...
static gboolean
gst_vtenc_get_allow_frame_reordering (GstVTEnc265 * self)
{
  gboolean result;
...
static void
gst_vtenc_set_allow_frame_reordering (GstVTEnc265 * self,
...
static gboolean
gst_vtenc_get_realtime (GstVTEnc265 * self)
{
  gboolean result;
...
static void
gst_vtenc_set_realtime (GstVTEnc265 * self, gboolean realtime)
{
  GST_OBJECT_LOCK (self);
...
static gdouble
gst_vtenc_get_quality (GstVTEnc265 * self)
{
  gdouble result;
...
static void
gst_vtenc_set_quality (GstVTEnc265 * self, gdouble quality)
{
  GST_OBJECT_LOCK (self);
...
static gint
gst_vtenc_get_max_keyframe_interval (GstVTEnc265 * self)
{
  gint result;
...
static void
gst_vtenc_set_max_keyframe_interval (GstVTEnc265 * self, gint interval)
{
  GST_OBJECT_LOCK (self);
...
static GstClockTime
gst_vtenc_get_max_keyframe_interval_duration (GstVTEnc265 * self)
{
  GstClockTime result;
...
static void
gst_vtenc_set_max_keyframe_interval_duration (GstVTEnc265 * self,
...
gst_vtenc_finish_encoding (GstVTEnc265 * self, gboolean is_flushing)
{
...
{
  GstVTEnc265 *self = GST_VTENC_265_CAST (enc);
...
GST_DEBUG_OBJECT (self, "Stop completed");
  return TRUE;
}

static CFStringRef
gst_vtenc_profile_level_key (GstVTEnc265 * self, const gchar * profile,
    const gchar * level_arg)
{
  ...
  key = g_strdup_printf ("H265_%s_%s", profile, level);
...
static gboolean
gst_vtenc_is_negotiated (GstVTEnc265 * self)
{
...
GST_DEBUG_OBJECT(self, "gst_vtenc_negotiate_downstream");
...
GST_DEBUG_OBJECT(self, "gst_vtenc_negotiate_downstream Completed");
  return result;
}

static void
gst_vtenc_clear_cached_caps_downstream (GstVTEnc265 * self)
{
...
gst_vtenc_create_session (GstVTEnc265 * self)
{
...
#if !HAVE_IOS
  const GstVTEncoder265Details *codec_details =
      GST_VTENC_265_CLASS_GET_CODEC_DETAILS (G_OBJECT_GET_CLASS (self));
...
static void
gst_vtenc_destroy_session (GstVTEnc265 * self, VTCompressionSessionRef * session)
{
...
static void
gst_vtenc_session_dump_properties (GstVTEnc265 * self,
    VTCompressionSessionRef session)
{
...
static void
gst_vtenc_session_configure_expected_framerate (GstVTEnc265 * self,
    VTCompressionSessionRef session, gdouble framerate)
{
...
static void
gst_vtenc_session_configure_max_keyframe_interval (GstVTEnc265 * self,
    VTCompressionSessionRef session, gint interval)
{
...
static void
gst_vtenc_session_configure_max_keyframe_interval_duration (GstVTEnc265 * self,
...
static void
gst_vtenc_session_configure_bitrate (GstVTEnc265 * self,
    VTCompressionSessionRef session, guint bitrate)
{
  gst_vtenc_session_configure_property_int (self, session,
...
static void
gst_vtenc_session_configure_allow_frame_reordering (GstVTEnc265 * self,
    VTCompressionSessionRef session, gboolean allow_frame_reordering)
{
...
static void
gst_vtenc_session_configure_realtime (GstVTEnc265 * self,
    VTCompressionSessionRef session, gboolean realtime)
{
...
static OSStatus
gst_vtenc_session_configure_property_int (GstVTEnc265 * self,
    VTCompressionSessionRef session, CFStringRef name, gint value)
{
  CFNumberRef num;
  OSStatus status;
...
static OSStatus
gst_vtenc_session_configure_property_double (GstVTEnc265 * self,
    VTCompressionSessionRef session, CFStringRef name, gdouble value)
{
  CFNumberRef num;
  OSStatus status;
...
static void
gst_vtenc_update_latency (GstVTEnc265 * self)
{
  OSStatus status;
  CFNumberRef value;
  int frames = 0;
...
GST_DEBUG_OBJECT(self, "gst_vtenc_update_latency");
  ...
  if (self->latency_frames == -1 || self->latency_frames != frames) {
    ...
    latency = frame_duration * frames * 5;
  ...
  CFRelease (value);
  GST_DEBUG_OBJECT(self, "gst_vtenc_update_latency Completed");
}

static GstFlowReturn
gst_vtenc_encode_frame (GstVTEnc265 * self, GstVideoCodecFrame * frame)
{
  CMTime ts, duration;
  ...
  GST_DEBUG_OBJECT (self, "Encode Frame");
...
GST_DEBUG_OBJECT (self, "Encode Frame Completed");
  return ret;

cv_error:
  {
  GST_DEBUG_OBJECT (self, "Encode Frame Error");
    gst_video_codec_frame_unref (frame);
...
{
  GstVTEnc265 *self = outputCallbackRefCon;
  GST_DEBUG_OBJECT(self, "gst_vtenc_enqueue_buffer");
...
beach:
  ...
  if (frame)
    ...
    GST_DEBUG_OBJECT(self, "gst_vtenc_enqueue_buffer Completed");
}

static gboolean
gst_vtenc_buffer_is_keyframe (GstVTEnc265 * self, CMSampleBufferRef sbuf)
{
  GST_DEBUG_OBJECT(self, "gst_vtenc_buffer_is_keyframe");
  gboolean result = FALSE;
...
GST_DEBUG_OBJECT(self, "gst_vtenc_buffer_is_keyframe Completed");
  return result;
}
...
static const GstVTEncoder265Details gst_vtenc_265_codecs[] = {
  {"H.265", "h265", "video/x-h265", kCMVideoCodecType_HEVC, FALSE},
#ifndef HAVE_IOS
  {"H.265 (HW only)", "h265_hw", "video/x-h265", kCMVideoCodecType_HEVC, TRUE},
#endif
};

void
gst_vtenc_265_register_elements (GstPlugin * plugin)
{
  guint i;

  GST_DEBUG_CATEGORY_INIT (gst_vtenc_265_debug, "vtenc265",
      0, "Apple VideoToolbox Encoder Wrapper");

  for (i = 0; i != G_N_ELEMENTS (gst_vtenc_265_codecs); i++)
    gst_vtenc_register (plugin, &gst_vtenc_265_codecs[i]);
}
